<x-layouts.app :title="$blog->title">
    ...
    <header class="relative text-foreground py-24 md:py-40 flex items-center justify-center text-center bg-cover bg-center" style="background-image: url('{{ $blog->thumbnail }}')">
        <div class="absolute inset-0 bg-gradient-to-t from-black/80 via-black/50 to-transparent"></div>
        <div class="relative container mx-auto px-6 animate-fade-in-up">
            <div class="max-w-4xl mx-auto">
                <div class="flex flex-wrap gap-2 justify-center mb-4">
                    @foreach($blog->categories as $category)
                        <span class="px-3 py-1 text-sm font-semibold bg-primary/80 rounded-full">{{ $category->name }}</span>
                    @endforeach
                </div>
    ...
    <div class="py-12 text-center bg-background/70">
        <a href="{{ route('blogs.index') }}" class="group inline-flex items-center text-primary font-semibold transition-all duration-300 hover:text-teal-300">
            <x-lucide-arrow-left class="mr-2 h-5 w-5 transform transition-transform duration-300 group-hover:-translate-x-1"/>
            Voltar para o Blog
        </a>
    </div>
